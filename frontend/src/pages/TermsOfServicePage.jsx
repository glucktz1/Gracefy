...
const TermsOfServicePage = () => {
  ...
  const [title, setTitle] = useState('Masharti ya Huduma');
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchContent = async () => {
      try {
        const res = await axios.get(`${API}/legal/terms_of_service`);
        const data = res.data;
        setTitle(data.title_sw || data.title || 'Masharti ya Huduma');
