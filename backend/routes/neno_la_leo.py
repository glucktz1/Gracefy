...
# Get database
def get_db():
    from core.database import get_db as db_getter
    return db_getter()
...
async def leader_login(data: LeaderLoginRequest):
    """Login for religious leaders"""
    db = get_db()
    
    leader = await db.religious_leaders.find_one({"email": data.email})
    if not leader:
        raise HTTPException(status_code=401, detail="Invalid credentials")
    
    if leader.get("password_hash") != hash_password(data.password):
        raise HTTPException(status_code=401, detail="Invalid credentials")
...
class PlayTrackRequest(BaseModel):
    neno_id: str
    audio_type: str = "reading"

@router.post("/play")
async def track_neno_play_body(data: PlayTrackRequest):
    """Track play count for Neno la Leo (JSON body version)"""
    db = get_db()
    
    audio_type = data.audio_type if data.audio_type in ["reading", "reflection"] else "reading"
    update_field = f"stats.{audio_type}_plays"
    
    result = await db.neno_la_leo.update_one(
        {"neno_id": data.neno_id},
        {
            "$inc": {
                update_field: 1,
                "stats.total_plays": 1
    ...
    neno = await db.neno_la_leo.find_one({"neno_id": data.neno_id})
    if neno:
